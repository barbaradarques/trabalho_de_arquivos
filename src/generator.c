...
/*
 * FUNÇÃO DE TESTE, DEVE SER DELETADO QUANDO INTEGRADO

int main (){
    FILE* series = fopen("series.txt", "r");
    FILE* db = fopen("db_series.bin", "wb");

    if(series != NULL && db != NULL){
        int i;
        for(i = 0; i < MAX_REG; i++){
            int pos = randomPosition();
            REG* regist = readToImport (series, pos);

            printf("\nregistro pos(%d) - status(%d):\n", 
                    pos, regist != NULL);
            if(regist != NULL){
                printf("ID - %d\n", getRegistID(regist));
                printf("TITLE - %s\n", getRegistTitle(regist));
                printf("DESCRIPTION - %s\n", getRegistDescrip(regist));
                printf("PRODUCER - %s\n", getRegistProducer(regist));
                printf("YEAR - %d\n", getRegistYear(regist));
                printf("SEASON - %d\n", getRegistSeason(regist));
                printf("GENRE - %s\n", getRegistGenre(regist));
            }

            printf("importing...\n");

            importOneRegist (db, regist);
            deleteRegist(&regist);
        }
        fclose(series);
        fclose(db);
